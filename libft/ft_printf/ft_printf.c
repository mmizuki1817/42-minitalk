...
/*   Updated: 2023/01/31 23:29:19 by mimatsub         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	ft_putstr_fd_printf(char *s)
{
	...
	if (s == NULL)
		return (ft_putstr_fd_printf("(null)"));
	count = 0;
	while (*s)
		count += write(1, s++, 1);
	return (count);
...
static size_t	ft_treat_something(char c, va_list ap)
{
	...
	count = 0;
	if (c == 'c')
		count = ft_putchar_fd_printf(va_arg(ap, int));
	else if (c == 's')
		count = ft_putstr_fd_printf(va_arg(ap, char *));
	else if (c == 'p')
...
static size_t	ft_count_output(const char *input, va_list ap)
{
	...
	while (input[i])
	{
		...
		{
			count += ft_putchar_fd_printf(input[i]);
		}
		i++;
	}
	return (count);
